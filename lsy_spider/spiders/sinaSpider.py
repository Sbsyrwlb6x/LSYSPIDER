...
from lsy_spider.weiboID import weiboID
from lsy_spider.scrapy_redis.spiders import RedisSpider
from scrapy.selector import Selector
from scrapy.http import Request
from lsy_spider.items import TweetsItem, InformationItem, RelationshipsItem
...
class Spider(RedisSpider):
    name = "lsy_spider"
    host = "https://weibo.cn"
    redis_key = "lsy_spider:start_urls"
    ...
    def start_requests(self):
        for uid in self.start_urls:
            ...
            #yield Request(url="http://weibo.cn/%s/info" % uid, callback=self.parse_information,meta={'proxy':'202.110.80.203:80'})
    ...
    def parse_tweets(self, response):
        ...
        for div in divs:
            try:
                ...
                tweetsItems["ID"] = ID
                tweetsItems["SystemTime"]=time.strftime("%Y-%m-%d %H:%M",time.localtime(time.time()))
                if content:
